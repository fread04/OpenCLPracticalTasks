...
private:
    ...
    const std::string kernelSource = R"(
        __kernel void reduce(__global const float* input, 
            ...
            __local float localBuffer[256]; // Local memory for reduction

            // Load data into local memory
            if (globalId < size) {
                localBuffer[localId] = input[globalId];
            } else {
                localBuffer[localId] = 0.0f;
            }
            ...
            if (localId == 0) {
                output[get_group_id(0)] = localBuffer[0];
            }
        }
    )";
...
public:
    ...
    float reduce(const std::vector<float>& input, double& executionTime) {
        ...
        size_t localSize = 256; // Must be a power of 2 for the kernel
