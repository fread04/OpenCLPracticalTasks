...
public:
    ...
    Matrix multiply(const Matrix& A, const Matrix& B, double& executionTime) {
        ...
        cl::Buffer bufferA(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
            A.data.size() * sizeof(float), const_cast<float*>(A.data.data()));
        cl::Buffer bufferB(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
            B.data.size() * sizeof(float), const_cast<float*>(B.data.data()));
        cl::Buffer bufferC(context, CL_MEM_WRITE_ONLY,
            C.data.size() * sizeof(float));

        // Set kernel arguments
        kernel.setArg(0, bufferA);
        kernel.setArg(1, bufferB);
        kernel.setArg(2, bufferC);
        kernel.setArg(3, static_cast<int>(A.rows));
        kernel.setArg(4, static_cast<int>(B.cols));
        kernel.setArg(5, static_cast<int>(A.cols));
        ...
        cl::NDRange globalSize(
            ((A.rows + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE,
            ((B.cols + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE
        );
        ...
        queue.enqueueReadBuffer(bufferC, CL_TRUE, 0,
            C.data.size() * sizeof(float), C.data.data());
